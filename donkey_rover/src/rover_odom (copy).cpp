            ...
            bool speed_dec_flag = false;
  			while(ros::ok()){
				...
				scannerCal = scannerRaw + scanner_offsetVal;
    				...
    				scan_quart.setRPY(((scannerCal/scannerCal_max)*roll_angle),0.0 , 0.0);
                    scan_quart_back.setRPY((-(scannerCal/scannerCal_max)*roll_angle),0.0 , 0.0);
                ...
                if (state == ESSIdle && scanner_horizontal)
                {
                        scannerCal = 0;
