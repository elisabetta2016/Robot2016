...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn2)
{
  ...
  /*
  float temp = fabs((float) laserCloudIn2->data[0]) + fabs((float) laserCloudIn2->data[1])*100 + fabs((float) laserCloudIn2->data[2])*10000;
  laserAngleCur = fabs((float) laserCloudIn2->data[0] / 10000) + fabs((float) laserCloudIn2->data[1]/100) + fabs((float) laserCloudIn2->data[2]/10) - 2.0;
  
  ROS_INFO("angle is %.1f in degree is :  %.1f",temp,laserAngleCur*180/M_PI);
  //laserAngleCur = ( (float)(laserCloudIn2->height) )/1000000.0 - 2.0;*/
  //laserAngleCur = Scanner_angle;
  memcpy(angle.b, &laserCloudIn2->data[0],4);
  //ROS_INFO("angle is %.1f in degree is :  %.1f",angle.f,laserAngleCur*180/M_PI);
  laserAngleCur = angle.f;
...
void OdomHandle(const nav_msgs::Odometry::ConstPtr& OdomIn)
{
  double roll, pitch, yaw;
  tf::Quaternion orientation;
  tf::quaternionMsgToTF(OdomIn->pose.pose.orientation, orientation);
  tf::Matrix3x3(orientation).getRPY(roll, pitch, yaw);
  
  imuPointerLast = (imuPointerLast + 1) % imuQueLength;

  imuTime[imuPointerLast] = OdomIn->header.stamp.toSec() - 0.1068;
...
/*
  ...
  imuVeloX[imuPointerLast] = OdomIn->twist.twist.linear.x;
  imuVeloY[imuPointerLast] = OdomIn->twist.twist.linear.y;
  imuVeloZ[imuPointerLast] = OdomIn->twist.twist.linear.z;
*/

  //adapted to the frame diff
  imuRoll[imuPointerLast]  = pitch;
  imuPitch[imuPointerLast] = yaw;
  imuYaw[imuPointerLast]   = roll;
  
  imuVeloX[imuPointerLast] = OdomIn->twist.twist.linear.y;
  imuVeloY[imuPointerLast] = OdomIn->twist.twist.linear.z;
  imuVeloZ[imuPointerLast] = OdomIn->twist.twist.linear.x;
  
  AccumulateOdomShift();  
}

void ScanInfo_callback(const donkey_rover::Rover_Scanner::ConstPtr& msg)
{
	Scanner_angle = msg->Scanner_angle;
}
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2> 
                                  ("/laser_cloud_in", 1, laserCloudHandler); ///sync_scan_cloud_filtered

  //ros::Subscriber subImu = nh.subscribe<sensor_msgs::Imu> ("/imu/data/", 5, imuHandler);
  ros::Subscriber subImu = nh.subscribe ("/odom", 1, OdomHandle);
  
  //Added
  ros::Subscriber subScanInfo = nh.subscribe("/RoverScannerInfo", 1, ScanInfo_callback);
  //End                         
