...
void cloud_outlier_removal(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_filtered)
{
  ...
  while (status) {
    ...
    if (newLaserCloudLast && newLaserOdometry && fabs(timeLaserCloudLast - timeLaserOdometry) < 0.005) {
      ...
      *laserCloudLast = *laserCloudCorner2 + *laserCloudSurf2;
      ...
      cloud_outlier_removal(laserCloudSurround,laserCloudSurround);
