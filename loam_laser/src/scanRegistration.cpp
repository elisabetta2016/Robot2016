...
void imuHandler(const sensor_msgs::Imu::ConstPtr& imuIn)
{
  ...
  tf::quaternionMsgToTF(odom_orientation, orientation);
...
{
  odom_orientation = OdomIn->pose.pose.orientation;
  ...
  try{
      listener.lookupTransform("/odom", "/laser",  
  ...
  */
  ...
  imuVeloX[imuPointerLast] = OdomIn->twist.twist.linear.x;//y
  imuVeloY[imuPointerLast] = OdomIn->twist.twist.linear.y + OdomIn->twist.twist.angular.z*0.493;//z
  imuVeloZ[imuPointerLast] = OdomIn->twist.twist.linear.z + OdomIn->twist.twist.angular.y*0.493;;//x
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subOdom = nh.subscribe ("/odom", 5, OdomHandle);
