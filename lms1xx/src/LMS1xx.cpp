...
void LMS1xx::getData(scanData& data) {
	...
	do {
		...
		if (retval) {
			len += read(sockDesc, buf + len, 200000 - len); //20000
		}
    ...
    //printf("numberEncoders: %s \n",tok);
	int NumberEncoders;
	sscanf(tok, "%d", &NumberEncoders);
    //printf("Nomber of encoders is %d and the ", NumberEncoders);
	...
	for (int i = 0; i < NumberChannels8Bit; i++) {
        ...
        tok = strtok(NULL, " "); //position variable
         //printf("position variable:%s \n",tok);
